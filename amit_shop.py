...
# ==============================================================================
BOT_TOKEN = "8714154207:AAFnZhjPHfjDNgqTxsJsRdeHpRyrZ694z1c"
BOT_USERNAME = "@amitxmodz_bot"
ADMIN_ID = 8187601070
ADMIN_CONTACT = "@FFH4X4MIT"
...
# LICENSE / EXPIRY
# ==============================================================================
# This build stops running 30 days after the start date below.
LICENSE_START_DATE = datetime(2026, 9, 7, 0, 0, 0)
LICENSE_VALID_DAYS = 30

def is_license_expired() -> bool:
    expiry_date = LICENSE_START_DATE + timedelta(days=LICENSE_VALID_DAYS)
    return datetime.now() >= expiry_date

def expiry_guard() -> None:
    if is_license_expired():
        logger.error("This bot build has expired.")
        raise SystemExit("This bot build has expired after 30 days.")
...
class GlobalSecurityMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler, event, data):
        # Runtime expiry check (covers incoming messages/callbacks too).
        if is_license_expired():
            return
...
def admin_kb() -> InlineKeyboardMarkup:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="⚙️ FamPay Setup", callback_data="admin_setup_fampay", icon_custom_emoji_id=get_emoji_icon("upi"), style="success"),
...
async def run_payment_verification(user_id: int, order_id: str, reply_target: Any) -> None:
    ...
    api_key_check = db_query("SELECT value FROM settings WHERE key='fampay_api'", fetchone=True)
    ...
    url = "https://fampay.anujbots.xyz/verify.php"
    params = {"order_id": order_id, "api_key": api_key, "cb": str(int(time.time() * 1000))}
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(url, params=params) as resp:
                ...
                real_status = str(res_json.get("status", "")).lower()

                if resp.status == 200 and real_status == "success" and res_json.get("data"):
                    txn_data = res_json.get("data", {})
                    utr = txn_data.get("utr") or txn_data.get("transaction_id") or "N/A"
                    ...
                    await send_advanced_notification(user_id, "DEPOSIT", txn[0], product=order_id, gateway="FamPay")
                    log_activity(user_id, "DEPOSIT_SUCCESS", f"Amount: {txn[0]}, Gateway: FamPay, Order: {order_id}, UTR: {utr}")
                else:
                    fail_msg = "⚠️ PAYMENT NOT RECEIVED YET. Please complete the payment first, then try Verify again."
            ...
            logger.error(f"FamPay API Error: {str(e)}")
            ...
            async with aiohttp.ClientSession() as session:
                try:
                    async with session.get(url, params=params) as resp:
                        ...
                        if resp.status == 200 and str(res_json.get("status", "")).lower() == "success" and res_json.get("data"):
                            txn_data = res_json.get("data", {})
                            utr = txn_data.get("utr") or txn_data.get("transaction_id") or "N/A"
                            db_query("UPDATE transactions SET status='paid' WHERE order_id=?", (order_id,))
                            ...
                            await send_advanced_notification(user_id, "DEPOSIT", amount, product=order_id, gateway="FamPay Auto")
                            log_activity(user_id, "DEPOSIT_AUTO_SUCCESS", f"Amount: {amount}, Gateway: FamPay Auto, Order: {order_id}, UTR: {utr}")
                except Exception as e:
...
async def process_fampay_payment_callback(call: CallbackQuery):
    ...
    await call.message.edit_text("⏳ <b>Generating Secure Link via FamPay...</b>", parse_mode='HTML')
...
async def generate_fampay_order(user_id: int, inr_amount: float, message_obj: Message) -> None:
    upi_check = db_query("SELECT value FROM settings WHERE key='fampay_upi_id'", fetchone=True)
    upi_id = upi_check[0] if upi_check and upi_check[0] else ""
    if not upi_id:
        return await message_obj.edit_text("⚠️ FamPay Gateway is currently offline. Admin needs to set the UPI ID (and API Key).", reply_markup=back_kb("gateway_inr"), parse_mode='HTML')
    ...
    async with aiohttp.ClientSession() as session:
        try:
            url = "https://fampay.anujbots.xyz/qr.php"
            params = {"upi": upi_id, "amount": f"{inr_amount:.2f}"}
            async with session.get(url, params=params) as resp:
                try: res_data = await resp.json(content_type=None)
                except: res_data = {}
                if resp.status == 200 and res_data.get("status") == "success" and res_data.get("data"):
                    order_data = res_data.get("data", {})
                    order_id = order_data.get("order_id", "")
                    qr_url = order_data.get("qr_url", "")
                    expires_at = order_data.get("expires_at_ist", "5 Minutes")
                else:
                    return await message_obj.edit_text(f"❌ <b>Gateway Data Error:</b> {res_data.get('message', f'HTTP {resp.status}')}", reply_markup=back_kb("gateway_inr"), parse_mode='HTML')
        except Exception as e:
    ...
    if not order_id or not qr_url:
        return await message_obj.edit_text("❌ <b>Gateway Error:</b> No order_id/QR returned by FamPay.", reply_markup=back_kb("gateway_inr"), parse_mode='HTML')
    ...
    try:
        ...
        pass
    try:
        await bot.send_photo(user_id, qr_url, caption=text, reply_markup=kb, parse_mode='HTML')
    except Exception:
...
async def process_buy(call: CallbackQuery):
    ...
    prod = db_query("""
        SELECT name, price_inr, stock, apk_link, validity, device_limit,
               ...
               external_product_id, requires_android_id, external_duration
...
async def setup_fampay_start(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID: return
    cur_upi = get_setting("fampay_upi_id", "Not set")
    cur_key = get_setting("fampay_api", "")
    cur_key_display = (cur_key[:10] + "••••••••") if cur_key else "Not set"
    await call.message.edit_text(
        "⚙️ <b>FAMPAY SECURITY DEPLOYMENT</b>\n"
        f"Current UPI ID: <code>{cur_upi}</code>\n"
        f"Current API Key: <code>{cur_key_display}</code>\n\n"
        "Input as: <code>yourid@fam|API_KEY</code>\n"
        "<i>(Only UPI changes? Just send</i> <code>yourid@fam</code><i>. Type /cancel to abort.)</i>",
        reply_markup=admin_back_kb(), parse_mode='HTML')
...
async def fampay_api(m: Message, state: FSMContext):
    ...
    raw = m.text.strip()
    parts = raw.split("|")
    new_upi = parts[0].strip()
    new_key = parts[1].strip() if len(parts) > 1 else None
    if "@" not in new_upi or " " in new_upi:
        await m.answer("❌ Invalid UPI ID! Format should be: <code>name@bank</code>", parse_mode='HTML')
        return
    db_query("INSERT OR REPLACE INTO settings (key, value) VALUES ('fampay_upi_id', ?)", (new_upi,))
    confirm = f"✅ <b>FamPay config updated:</b>\n• UPI ID: <code>{new_upi}</code>"
    if new_key:
        db_query("INSERT OR REPLACE INTO settings (key, value) VALUES ('fampay_api', ?)", (new_key,))
        confirm += f"\n• API Key: <code>{new_key[:10]}••••••••</code>"
    confirm += "\n\n<i>All future QR generation + verification will use this UPI/API.</i>"
    await m.answer(confirm, reply_markup=admin_kb(), parse_mode='HTML')
    await state.clear()
...
if __name__ == "__main__":
    try:
        ...
        # developer:- @ntxakash
        # IF you Remove anything code will give multiple error